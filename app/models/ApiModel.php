...
class ApiModel
{
    ...
    public function middleware_auth($token)
    {
        ...
        if(!isset($token)) {
            write_log('tracking', 'Error', 'Le token n\'est pas renseigné', 'red');
        ...
        if(isset($token)) {
            ...
            if(preg_match($pattern, $token, $matches)) {
                write_log('tracking', 'Success', 'Le token n\'est pas un bearer', 'green');
                $token = $matches[1];
            } else {
        ...
        if(!$token_in_db) {
            write_log('tracking', 'Error', 'Le token n\'est renseigné en base de données', 'red');
            http_response_code(401);
        ...
        if($token_in_db['value'] !== $token) {
            write_log('tracking', 'Error', 'Le token n\'est pas valide', 'red');
            http_response_code(401);
            echo json_encode(['error' => 'L\'authentification a échoué']);
            return false;
        }
